...
$script:LogPath = Join-Path $env:TEMP 'DatasetCapture_install.log'
$script:ShortcutName = '数据集采集.lnk'
...
function Install-DriverInf {
    ...
    $output = (& (Join-Path $env:SystemRoot 'System32\pnputil.exe') /add-driver $InfPath /install 2>&1)
...
function New-Shortcut {
    ...
    $shortcut.Description = '数据集采集工具'
    $shortcut.Save()
}

function Main {
    Write-Log '开始安装数据集采集工具。'
    if (-not [Environment]::Is64BitOperatingSystem) {
        throw '本程序仅支持 64 位 Windows。'
    }

    $packageRoot = $PSScriptRoot
    $appDir = Join-Path $packageRoot 'DatasetCapture'
    $exePath = Join-Path $appDir 'DatasetCapture.exe'
    if (-not (Test-Path -LiteralPath $exePath -PathType Leaf)) {
        if (Test-Path -LiteralPath (Join-Path $packageRoot 'DatasetCapture.exe') -PathType Leaf) {
            $appDir = $packageRoot
            $exePath = Join-Path $appDir 'DatasetCapture.exe'
        } else {
            throw '未找到 DatasetCapture.exe。请保持发布包目录完整，不要只复制 EXE。'
        }
    }
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $appDir 'dataset'))) {
        New-Item -ItemType Directory -Force -Path (Join-Path $appDir 'dataset') | Out-Null
    }
    ...
    New-Shortcut -ShortcutPath (Join-Path $desktop $script:ShortcutName) -TargetPath $exePath -WorkingDirectory $appDir
    New-Shortcut -ShortcutPath (Join-Path $programs "数据集采集\$script:ShortcutName") -TargetPath $exePath -WorkingDirectory $appDir
